...
export const index = {
  async get (req, res) {
    try {
      console.log("im in the index getting" + req.params.username)
      if (req.params.username) {
        console.log("looking for user")
        let user = await User.find({ username : req.params.username })
        console.log("found user")
        if (!user) throw new ServerError(`Username ${req.params.username} does not exist`)
        console.log("returning " + user)
        res.json(user)
      } else {
        let users = await User.find({})
        if (!users) throw new ServerError(`No users exist at the moment`, { status: 404 })
        res.json(users)
      }
    } catch (error) {
