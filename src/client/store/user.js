...
export const mutations = {
  LOVE_REQUEST (state) {
    console.log('Love pending...')
  },
  LOVE_SUCCESS (state, data) {
    state.love_list = data.user.love_list
    console.log('Love success!')
  },
  LOVE_FAILURE (state, error) {
    console.log('Love Failure.', error.response.data)
    console.error(error)
  },
...
export const actions = {
  async love ({ commit }, payload) {
    try {
      commit('LOVE_REQUEST')
      let { data } = await axios.post('/users/love', payload)
    } catch (error) {
      commit('LOVE_FAILURE', error)
      commit('notification/FAILURE', error.response.data, { root: true })
    }
  },
  async bookmark ({ commit }, payload) {
    try {
      let { data } = await axios.post('/users/bookmark', payload)
